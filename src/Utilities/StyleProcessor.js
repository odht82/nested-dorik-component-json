        ...
        for (let i = 0; i < elem?.length; i++) {
            if (elem[i].content) {
                ...
                for (let n in elem[i].style) {
                    allStyles.push(styleProperties(n.replace(/([a-z])([A-Z])/g, "$1-$2").toLowerCase(), elem[i].style[n]))
                }
            ...
            if (elem[i].form) {
                const gatherStyles = (className, styleObject) => {
                    ...
                    for (let n in styleObject) {
                        allStyles.push(styleProperties(n.replace(/([a-z])([A-Z])/g, "$1-$2").toLowerCase(), styleObject[n]));
                    }
